...
`dotnet run` performs NuGet restore before compiling. The project has no third-party NuGet packages, but NuGet still tried to reach the configured `https://api.nuget.org/v3/index.json` source (including vulnerability/audit metadata). A corporate firewall/socket policy blocked that request. v1.0.2 adds a local `NuGet.Config` that clears external package sources and disables NuGet audit for this project.
